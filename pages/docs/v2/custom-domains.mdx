...
**Apex domains** can be configured with a **A** record, while **subdomains** can be configured with a **CNAME** record.
Both domain types can also be configured using the **Nameservers** method.

#### Apex Domains

You can configure [apex domains](/docs/v2/platform/glossary#apex-domain) either with a **A** record or through the **Nameservers** method.
...
<Image
  ...
  shadow
/>
<Caption>A <b>Wildcard Domain</b> being deployed.</Caption>
...
When a custom domain is assigned to a project with an enabled [<ProductName /> for Git Integration](/docs/v2/git-integrations), each push (including merges) to the [Production Branch](/docs/v2/git-integrations#production-branch) (commonly `master`) will trigger a deployment to the defined domain.
...
Every commit pushed to the [Production Branch](/docs/v2/git-integrations#production-branch) of your Git repository using our [Git Integration](/docs/v2/git-integrations) will be assigned the Domains [configured in your project](#adding-a-domain).
