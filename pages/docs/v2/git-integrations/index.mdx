...
The easiest way to use a Git Integration is to think of your **`master` branch as production**. Every time a pull/merge request is made to that branch, <ProductName /> will **create a unique deployment**, allowing you to **view the changes in a preview environment before merging**.

In this case, `master` is the Production Branch. When merging to the Production Branch, a **Production Deployment** is made, making the latest changes available to assigned custom domains automatically.

You can choose to use another branch, or the default branch as the [Production Branch](#production-branch).
...
You can install any Git Integration directly from your dashboard by clicking ["Import Project"](/import) followed by ["Import Git Repository"](/import/git). Then enter the URL of your GitHub, GitLab, or Bitbucket repository to connect and import.
...
<Image
  ...
  borderRadius
/>
<Caption>Installing the Git Integration for a Git provider of your choice.</Caption>
...
A [Production Deployment](/docs/v2/platform/deployments#production) will be created each time you merge to the **Production Branch**. When you connect a new repository to <ProductName />, `master` will be the Production Branch. If `master` doesn't exist, the default branch will be the Production Branch.

You can change the configuration to Repository Default, `master`, or any other custom branch from the Git Integrations tab in your Project Settings page.
